...
# 生成输出文件名
output_filename = f'result_fibon/fibon_cnn_FL{NUM_ROUNDS}_task{tasknum}_epoch{EPOCHS}_bs{BATCH_SIZE}_learnrate{LEARNING_RATE}_client{NUM_CLIENTS}.txt'
detailed_results_filename = f'result_fibon/detailed_results_task{tasknum}_final_round.txt'

# 1. 读取CSV文件
input_filename = f'1csv_fibon/fibon_output_task{tasknum}.csv'
...
# 更新特征列名以匹配新数据
features = ['Concurrent task count', 'Total Fibonacci size', 'New task start time (relative)',
            'New task Fibonacci size', 'New task FLOPs', 'CPU frequency (GHz)']
...
# 计算每行中 'Task start times' 和 'Task operations' 的平均值
df['Avg task start time'] = df['Task start times'].apply(lambda x: np.mean(x))
df['Avg task operations (GFLOPs)'] = df['Task operations (FLOPs)'].apply(lambda x: np.mean(x))

# 对主要特征进行标准化处理
features_with_avg = features + ['Avg task start time', 'Avg task operations (GFLOPs)']
df[features_with_avg] = scaler.fit_transform(df[features_with_avg])

# 提取所有特征
all_features = df[features_with_avg].values
