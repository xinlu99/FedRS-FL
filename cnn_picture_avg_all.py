...
# 生成输出文件名
output_filename = f'result_picture/hahamul_avg_picture_FL{NUM_ROUNDS}_task{tasknum}_epoch{EPOCHS}_bs{BATCH_SIZE}_learnrate{LEARNING_RATE}_client{NUM_CLIENTS}.txt'
detailed_results_filename = f'result_picture/mul_detailed_avg_results_task{tasknum}_final_round.txt'

# 1. 读取训练数据的CSV文件
input_filename = f'csv_picture/picture_avg_task_multiple.csv'
df = pd.read_csv(input_filename)

test_filename = f'csv_picture/picture_avg_task{tasknum}.csv'  # 替换为实际的测试文件路径
...
train_labels = df[['Total latency (excluding new task)', 'New task latency']].values

# 读取测试数据的CSV文件，并随机抽取20%作为测试集
#test_filename = 'csv_picture/test_picture.csv'  # 替换为实际的测试文件路径
...
test_labels = test_df[['Total latency (excluding new task)', 'New task latency']].values
