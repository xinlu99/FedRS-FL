...
# 生成输出文件名
output_filename = f'result_picture/picture_tcn{NUM_ROUNDS}_task{tasknum}_client{NUM_CLIENTS}.txt'
...
# 将数据转换为TCN输入格式
def create_tcn_dataset(data, labels, timesteps):
    X, y = [], []
...
# 创建TCN数据集
X, y = create_tcn_dataset(all_features, labels, TIMESTEPS)
...
class TCNModel(nn.Module):
    ...
    def forward(self, x):
        x = x.transpose(1, 2)  # TCN expects input of shape (N, C, L), where N=batch size, C=channels, L=sequence length
        out = self.network(x)
        out = self.fc(out[:, :, -1])
        return out
...
output_size = 2
num_channels = [25] * 4  # 可以调整此列表设置每层的通道数
kernel_size = 2
dropout = 0.3
...
for round_num in range(NUM_ROUNDS):
    ...
    for i in range(NUM_CLIENTS):
        ...
        # 每个客户端初始化自己的模型
        client_model = TCNModel(input_size, output_size, num_channels, kernel_size, dropout).to(device)
    ...
    global_model = TCNModel(input_size, output_size, num_channels, kernel_size, dropout).to(device)
