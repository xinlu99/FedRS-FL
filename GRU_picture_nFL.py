...
# 生成输出文件名
output_filename = f'result_picture/picture_gru{NUM_ROUNDS}_task{tasknum}_client{NUM_CLIENTS}.txt'
...
# 将数据转换为GRU输入格式
def create_gru_dataset(data, labels, timesteps):
    X, y = [], []
...
# 创建GRU数据集
X, y = create_gru_dataset(all_features, labels, TIMESTEPS)
...
hidden_size = 50  # 调整隐藏单元数
num_layers = 3  # 调整GRU层数
output_size = 2
...
for round_num in range(NUM_ROUNDS):
    ...
    for i in range(NUM_CLIENTS):
        ...
        # 每个客户端初始化自己的模型
        client_model = GRUModel(input_size, hidden_size, num_layers, output_size).to(device)
    ...
    global_model = GRUModel(input_size, hidden_size, num_layers, output_size).to(device)
