...
# 输入的CSV文件夹路径列表
folders = [f'csv_picture_{i}/' for i in range(1, 10)]  # 假设你要读取csv_picture_1到csv_picture_9文件夹

# 输出的CSV文件路径和文件名
output_csv = f'csv_picture/picture_avg_task_multiple.csv'
...
for input_folder in folders:
    for filename in os.listdir(input_folder):
        if filename.endswith('.csv'):
            print(f"Processing file: {filename} in folder: {input_folder}")
